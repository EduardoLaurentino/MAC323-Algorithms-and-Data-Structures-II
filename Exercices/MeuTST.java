...
public class MeuTST<Value extends Comparable<Value>> {
    ...
    public Iterable<String> keysWithPrefixByValue(String prefix) {
    	if (prefix == null) throw new IllegalArgumentException("argument to keysWithPrefixByValue() is null");

    	ST<Value, String> st = new ST<Value, String>();

        for(String key : keysWithPrefix(prefix))
        	st.put(get(key), key);

        Stack<String> pre_retorno = new Stack<String>();
        for (Value s : st.keys()){
        	pre_retorno.push(st.get(s));
        }

        Queue<String> retorno = new Queue<String>();
        while(!pre_retorno.isEmpty()){
        	retorno.enqueue(pre_retorno.pop());
        }
    ...
    /*private Node<Value> delete(Node<Value> x, String key, int level, int d) {
        if (x == null) return null;
        if (d == key.length()) {
            if (x.val != null) n--;
            x.val = null;
        	...
        	if (key.charAt(level) < (x.c))
        		delete(x.left, key, level, d);
        	else if (key.charAt(level) > x.c)
        		delete(x.right, key, level, d);
        	else if (key.charAt(level) == x.c) {
        		if (delete(root.mid, key, level+1,n) != null){
        			if (!isLeaf(root) && isFreeNode(root)) return x;
        			else return null;
        		}
        	}
        }
